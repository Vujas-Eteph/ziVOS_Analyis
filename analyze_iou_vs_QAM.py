...
def arguments_parser():
    ...
    parser.add_argument("--method", type=str, default='QDMN',
...
def get_masked_entropy(
    ...
    kernel = gen_kernel(value_for_mask_H, tp.sum() + fx.sum())
...
def foo(sequence_data):
    (load_data, get_masked_entropy, args, op_func,
     ...
     QAM_directory, confusion_directory,
    ...
    QAM_r = open(os.path.join(QAM_directory, dataset_name, sequence_name + '.json'))
    QAM_r = json.load(QAM_r)
    ic(QAM_r)
    if confusions_r is None or QAM_r is None:  # skip sequence if no data
        return None

    if confusions_r.shape[1] != len(QAM_r.keys()):
        raise(f"Confusion ({len(confusions_r)}) and Entropy "
              f"Error: ({len(QAM_r)}) don't cover the same amount of frames")
    ...
    ic(gt_with_a_counterpart_pd)
    ic(image_with_a_counterpart_pd)
    ic(predictions_file_names)
    # - Loop over images/annotations in sequence ---
    for fdx, (gt_mask_name, image_name, prediction_name) in enumerate(zipped_inputs):
        sum = 0
        objects = 0
        for elem in QAM_r[str(fdx)].values():
            sum += elem
            objects += 1
        QAM_score = sum/objects
        ...
        for obx in total_nbr_of_objects:
            ...
            ic(QAM_r[str(fdx)])
            try:
                QAM_score_fdx_obx = QAM_r[str(fdx)][str(obx)]
            except KeyError:
                QAM_score_fdx_obx = None
            ...
            frame_data[f'object_{obx}'] = {
                    ...
                    'IoU': IoU,
                    # QAM scores
                    'QAM_score_fdx': QAM_score,
                    'QAM_score_fdx_obx': QAM_score_fdx_obx,
                }
...
if __name__ == "__main__":
    ...
    QAM_directory = config.QDMN_variant_loader()
    ...
    ic(QAM_directory, confusion_directory)
