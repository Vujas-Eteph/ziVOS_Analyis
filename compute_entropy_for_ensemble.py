...
def compute_entropy(sequence_data):
    ...
    dict_for_the_entropies = dict()
    
    all_softmax_outputs = all_softmax_outputs[::-1]
    all_entropy_outputs = all_entropy_outputs[::-1]
    ...
    for edx, (softmax_dir, entropy_dir) in enumerate(zip(all_softmax_outputs, all_entropy_outputs)):
        ...
        for fdx, (gt_mask_name, image_name, pd_mask_name, softmax_name) in enumerate_data:
            ...
            if edx == 0:
                if fdx == 0:
                    aleatoric = np.zeros([len(groundtruth_file_names),
                                        *efficient_entropy.shape])
                aleatoric[fdx, :, :, :] = efficient_entropy
            elif edx < total_number_of_models:
                aleatoric[fdx, :, :, :] += efficient_entropy
            else:
                continue
    ...
    aleatoric /= (total_number_of_models -1)
    epistemic = entropy_for_sequence - aleatoric
    
    epistemic_dir = entropy_directory.replace("Entropy", "epistemic")
    aleatoric_dir = entropy_directory.replace("Entropy", "aleatoric")
    create_directory_if_not_there(epistemic_dir)
    create_directory_if_not_there(aleatoric_dir)
    dir_to_save_epistemic = os.path.join(epistemic_dir, f'{sequence}')
    dir_to_save_aleatoric = os.path.join(aleatoric_dir, f'{sequence}')
    data_exporter.save_data(epistemic, sequence,
                            dir_to_save_epistemic)
    data_exporter.save_data(aleatoric, sequence,
                            dir_to_save_aleatoric)
